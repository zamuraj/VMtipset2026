...
## $(date +%Y-%m-%d) - Consistent Focus States on Icon Buttons
**Learning:** Icon-only buttons lacking `outline-none` and `focus:ring-2` can silently fail accessibility checks because default browser outlines might be removed by Tailwind resets. When fixing these, it is crucial to scan the *entire* codebase (e.g., via `grep`) to bundle similar a11y findings into a single commit to avoid fragmented PRs.
**Action:** Always verify all instances of a pattern (like `<button>` elements with `aria-label` but without `focus:ring`) across the codebase, and bundle these related fixes together.
