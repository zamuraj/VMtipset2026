...
import { 
  ...
  Swords, Bell, PlayCircle, Banknote, MessageSquare, Send, Goal, ShieldCheck, ChevronDown, ChevronUp, MapPin, ListOrdered
} from 'lucide-react';
...
const VM_SCHEDULE = [
  ...
  "33|20 jun 19:00|F|Nederländerna|Frankrike|Houston", "34|20 jun 22:00|E|Uruguay|Tyskland|Toronto",
...
// Mock för att fylla ut raderna i demon
...
export default function App() {
  ...
  const groupStandings = useMemo(() => {
    ...
    Object.keys(TEAMS).forEach(name => {
      stats[name] = { name, played: 0, win: 0, draw: 0, loss: 0, gf: 0, ga: 0, gd: 0, pts: 0, group: "" };
    });
  ...
  const matchStats = useMemo(() => {
    return matches.map(m => {
      ...
      if (res) {
        const okPct = pct[res];
        if (okPct < 25) comm = `🔥 SKRÄLL! Endast ${okPct}% fick poäng här.`;
  ...
  const submitRegistration = () => {
    ...
    setRegStep(1);
    setRegName(''); setRegEmail(''); setRegGoals(''); setRegPicks({});
  };
        ...
        {!showRegister ? (
          <>
            <div className="flex bg-black/40 rounded-xl p-1.5 mb-8 border border-white/10 shadow-inner">
              ...
              <button onClick={() => setShowRegister(true)} className="flex-1 py-3 text-sm font-bold rounded-lg text-slate-400 hover:text-white transition-all">Lämna Tips</button>
            </div>
          ...
          <div className="animate-in fade-in slide-in-from-right-4 duration-300">
            {regStep === 1 ? (
              <form onSubmit={e => { e.preventDefault(); setRegStep(2); }} className="space-y-4">
                ...
                <input type="number" value={regGoals} onChange={e=>setRegGoals(e.target.value)} placeholder="Skiljefråga: Totalt antal mål?" className="w-full p-4 rounded-2xl bg-black/40 border border-white/10 outline-none focus:border-emerald-500 font-bold" required />
                <button type="submit" className="w-full py-4 bg-emerald-600 hover:bg-emerald-500 text-white font-black rounded-2xl shadow-lg mt-4">NÄSTA: TIPPA MATCHER</button>
              </form>
            ) : (
