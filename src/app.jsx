  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 pb-20">
      ...
      <nav aria-label="Huvudmeny" className="fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-md border-t z-40 flex justify-around p-1 sm:sticky sm:top-[72px] sm:max-w-5xl sm:mx-auto sm:my-4 sm:rounded-3xl sm:border shadow-xl">
        <button onClick={() => navigateTab('leaderboard')} aria-current={activeTab==='leaderboard' ? 'page' : undefined} aria-label="Leaderboard" title="Leaderboard" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='leaderboard'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><Trophy/></button>
        <button onClick={() => navigateTab('groups')} aria-current={activeTab==='groups' ? 'page' : undefined} aria-label="Grupper" title="Grupper" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='groups'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><ListOrdered/></button>
        <button onClick={() => navigateTab('h2h')} aria-current={activeTab==='h2h' ? 'page' : undefined} aria-label="Head 2 Head" title="Head 2 Head" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='h2h'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><Swords/></button>
        <button onClick={() => navigateTab('chat')} aria-current={activeTab==='chat' ? 'page' : undefined} aria-label="Snackis" title="Snackis" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='chat'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><MessageSquare/></button>
        <button onClick={() => navigateTab('matches')} aria-current={activeTab==='matches' ? 'page' : undefined} aria-label="Matcher" title="Matcher" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='matches'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><CalendarDays/></button>
        <button onClick={() => navigateTab('hof')} aria-current={activeTab==='hof' ? 'page' : undefined} aria-label="Hall of Fame" title="Hall of Fame" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='hof'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><History/></button>
        {currentUser.isAdmin && <button onClick={() => navigateTab('admin')} aria-current={activeTab==='admin' ? 'page' : undefined} aria-label="Admin" title="Admin" className={`p-4 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500/50 transition-colors ${activeTab==='admin'?'text-indigo-600':'text-slate-300 hover:text-slate-400'}`}><Settings/></button>}
      </nav>
