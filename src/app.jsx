              ...
              <div className="space-y-3">
                {hallOfFame.map(entry => (
                  <div key={entry.id} className="flex items-center justify-between p-4 rounded-2xl border bg-slate-50/50">
                    ...
                    <button onClick={() => { if(window.confirm('Är du säker på att du vill ta bort denna Hall of Fame-post?')) deleteDoc(doc(db, 'hallOfFame', entry.id)) }} aria-label="Ta bort" title="Ta bort" className="p-2 text-red-400 hover:bg-red-50 rounded-xl transition-colors"><Trash2 size={16}/></button>
                  </div>
                ))}
              </div>
            </div>
